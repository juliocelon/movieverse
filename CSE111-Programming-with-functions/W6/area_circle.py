...
def populate_main_window(frm_main):
    ...
    Return: nothing
    """
    # Create a label that displays "Ratio:"
    lbl_ratio = Label(frm_main, text="Ratio:")

    # Create an integer entry box where the user will enter the ratio
    ent_ratio = IntEntry(frm_main, width=4, lower_bound=1, upper_bound=100)

    # Create a label that displays "meters"
    lbl_meters_units = Label(frm_main, text="meters")

    # Create a label that displays "Area:"
    lbl_area = Label(frm_main, text="Area:")

    # Create labels that will display the results.
    lbl_area_result = Label(frm_main, width=3)
    lbl_rate_units = Label(frm_main, text="square meters")
    ...
    lbl_ratio.grid(         row=0, column=0, padx=1, pady=1)
    ent_ratio.grid(         row=0, column=1, padx=3, pady=3)
    lbl_meters_units.grid(  row=0, column=2, padx=0, pady=3)
    ...
    lbl_message.grid(    row=2, column=2, padx=0, pady=3)
    ...
    def calculate(event):
        ...
        try:
            ...
            lbl_message.config(text="Invalid input. Please enter a valid ratio.")
